<?php
class Zend_Controller_Action_Helper_ClassUpload extends Zend_Controller_Action_Helper_Abstract {
            ...
            if (!$this->file_src_mime || !is_string($this->file_src_mime) || empty($this->file_src_mime) || strpos($this->file_src_mime, '/') === FALSE) {
                if ($this->mime_magic) {
                    ...
                    if (function_exists('mime_content_type')) {
                        ...
                        if (preg_match("/^[a-z0-9]([0-9a-z])+$/i", $this->file_src_mime)) {
                            $this->file_src_mime = preg_replace("/^([\.-\w]+)\/([\.-\w]+)(.*)$/i", '$1/$2', $this->file_src_mime);
                            $this->log .= '-&nbsp;MIME validated as ' . $this->file_src_mime . '<br />';
                        } else {
                            $this->file_src_mime = null;
