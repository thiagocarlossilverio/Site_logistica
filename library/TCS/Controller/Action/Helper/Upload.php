...
class Zend_Controller_Action_Helper_Upload extends Zend_Controller_Action_Helper_Abstract {
    ...
    public function Upload($campo, $config = NULL, $marca_agua = FALSE) {
        ...
        if ($info['type'] == 'image' && in_array($info['extensao'], $info['extAceitas'])) {
          ...
          /*  if (isset($config->img) && $config->img) {
            ...
            if (is_numeric($config['Y']) > 0 || is_numeric($config['X']) > 0) {
                 $img->image_resize = true;   // Aceitar redimensionamento.
                $img->image_ratio = true;    // Manter a Propor��o
                $img->image_y = $config['Y']; // Defino um Array no TOPO da Classe, passando Y Maximo da Imagem
                $img->image_x = $config['X']; // Defino um Array no TOPO da Classe, passando X Maximo da Imagem
            }

            //if($marca_agua){
                $img->image_watermark = '/home/storage/f/91/b0/piglog/public_html/assets/images/logo1.png';
