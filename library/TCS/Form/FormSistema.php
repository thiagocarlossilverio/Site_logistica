...
//        $elemento = $this->createElement('text', 'cor_menu', array('id' => 'picker2', 'Label' => 'Cor do Menu', 'class' => 'form-control'));
//        $this->addElement($elemento);

        $elemento = $this->createElement('submit', 'Salvar', array('class' => 'btn btn-success'));
        $elemento->removeDecorator('Label');
        $this->addElement($elemento);
        $this->addDisplayGroup(array('id', 'titulo', 'logo', 'cor_layout', 'cor_menu', 'Salvar'), 'confsistema', array('removeDecorator' => 'Label'));
